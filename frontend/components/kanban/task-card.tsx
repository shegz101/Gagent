...
const TaskCard: React.FC<TaskCardProps> = ({ task, onDragStart, onDragEnd, onStatusChange, onClick }) => {
  ...
  return (
    <div
      ...
      className={`task-card p-5 bg-card rounded-lg border border-border shadow-sm hover:shadow-md transition-all duration-200 min-h-[200px] flex flex-col cursor-pointer hover:border-primary/50 ${isDragging ? 'opacity-50' : ''}`}
    >
      {/* Header with tag and due date */}
      <div className="flex justify-between items-start mb-4 flex-shrink-0">
        <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${getTagClass()}`}>
          {task.tag.label}
        </span>
        <span className="text-muted-foreground text-sm">{task.dueDate}</span>
      </div>
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        <div className="flex-1 mb-4">
          <h5 className="font-semibold mb-2 text-foreground text-base leading-tight line-clamp-2">{task.title}</h5>
          <p className="text-sm text-muted-foreground line-clamp-3 leading-relaxed">{task.description}</p>
        </div>
